...
export function registerLogcatTools(server: McpServer) {
  server.tool(
    "clear_logs",
    "Clear the logcat buffer on the emulator",
  ...
  server.tool(
    "capture_logs",
    "Capture logcat output for 5 seconds, optionally filtered by tag or priority",
    {
      serial: z.string().optional(),
  ...
  server.tool(
    "get_crash_log",
    "Capture any FATAL EXCEPTION crash logs from logcat",
    { serial: z.string().optional() },
    async ({ serial }) => {
      const baseArgs = serial ? ["-s", serial] : [];
