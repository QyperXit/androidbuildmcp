...
export function registerEmulatorTools(server: McpServer) {
  server.tool(
    "list_emulators",
    "List all available Android Virtual Devices (AVDs) and currently running emulators",
    {},
    async () => {
  ...
  server.tool(
    "wait_for_boot",
    "Wait until the emulator has fully booted. Polls every 3 seconds up to 120 seconds.",
    {
      serial: z
        .string()
        .optional()
