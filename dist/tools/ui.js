...
export function registerUiTools(server) {
    server.tool("tap", "Tap at a specific coordinate on the emulator screen", {
    ...
    server.tool("swipe", "Swipe from one coordinate to another", {
        x1: z.number(),
        y1: z.number(),
        x2: z.number(),
        y2: z.number(),
    ...
    server.tool("press_key", "Press an Android keycode", {
    ...
    server.tool("dump_ui", "Dump the UI hierarchy as XML so you can find element coordinates and resource IDs", { serial: z.string().optional() }, async ({ serial }) => {
