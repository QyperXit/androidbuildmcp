...
export function registerBuildTools(server) {
    server.tool("build_debug", "Build a debug APK using Gradle", { projectPath: z.string().describe("Absolute path to the Android project root (where gradlew lives)") }, async ({ projectPath }) => {
        const result = await execa("./gradlew", ["assembleDebug"], { cwd: projectPath });
        return {
            content: [{ type: "text", text: result.stdout + "\n" + result.stderr }],
        };
    });
    server.tool("build_variant", "Build a specific Gradle variant (e.g. stagingDebug, productionRelease)", {
    ...
    server.tool("clean_build", "Run Gradle clean to delete all build outputs", { projectPath: z.string().describe("Absolute path to the Android project root") }, async ({ projectPath }) => {
